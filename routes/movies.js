...
router.post("/movie/create",isAdmin,function(req,res)
{
  ...
  const newmovie={name:name,img:img,ratings:ratings,director:director,summary:summary,release:release,duration:duration};
...
const url="http://www.omdbapi.com/?i="+req.params.id+"&apikey=fbc34bf1";

request({url: url, json: true},function(err,movie,response){
  if(err)
    console.log(err)
  else {
    const duration=(movie.body.Runtime.slice(" "));
    const release=movie.body.Released.split(" ");
    const newmovie={
